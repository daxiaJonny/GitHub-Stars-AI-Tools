...
`用户问题 → 意图门控 → AI 查询理解（可选） → Query Embedding（可选） → zvec Top 30 候选 → 严格关键词融合 → AI 白名单筛选 → 0 到 10 条结果与同源回答`
...
- 关键词候选必须达到最低分；AI 可用时向量层保留 Top 30 交给 AI 筛选，AI 不可用时向量候选必须达到 minScore。
- ASCII 技术词按完整词边界匹配；`AI`、`RAG`、`UI` 等短缩写不做宽泛 AI 扩写，向量候选必须同时有精确词命中。
- 多词关键词查询必须命中至少两个有效词，五个以上检索词至少命中三个；上下文只能对已命中候选加分。
- AI 只能从真实候选池中筛选，最终结果为 0 到 10 条；相关项目不足时不得凑数。
- 验收：`你好` 返回 0 条；无关词不返回全库；语义同义查询能由向量召回；向量失败可回退。

### P10.5 RAG 回答与界面（P1 / M）

- 响应增加检索模式、向量状态、错误降级信息和回答正文。
- AI 回答只使用候选池的仓库名、描述、Topics 和摘要，不发送个人笔记。
- AI 必须返回结构化仓库全名列表；后端校验候选白名单后，用同一列表重排并裁剪右侧结果。
- 验收：回答和右侧仓库顺序、数量一致，空结果不编造项目，历史会话可兼容读取。
...
- 真实仓库验收：[本地向量检索真实仓库验收](../quality/local-vector-search-acceptance.md)。
- 完成判定：所有自动化测试通过，`你好` 为 0 条，任何最终搜索 `totalCount <= 10`，zvec smoke test 能关闭重开后保持排序。
