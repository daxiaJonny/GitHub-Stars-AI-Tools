...
| Knowledge Pipeline | 摘要、翻译、关键词、Embedding | README 原文、仓库元数据 | 中文摘要、中文译文、向量、关键词 | 可断点续跑，按内容 hash 幂等 |
| Search Engine | 关键词、过滤、语义检索、重排序 | 查询词、过滤条件 | 结果列表、匹配理由 | 不直接调用 GitHub，不直接调用 LLM 生成正文 |
| AI Provider Layer | 翻译、摘要、Embedding、查询理解 | 标准化 AI 请求 | 标准化 AI 响应 | 屏蔽 MiniMax/OpenAI/DeepSeek/Ollama 差异 |
| Storage | 本地持久化、索引、任务状态 | 模块写入请求 | SQLite/FTS/向量索引 | 数据分层，禁止混写 |
...
| Ports over Implementation | GitHub、AI、向量库都走 Provider 接口 | 如果直接绑定单一模型，后续切换 MiniMax 成本高 |
| Environment-Agnostic | 领域逻辑不依赖 Tauri/Electron/Web | 如果把业务逻辑写进桌面壳，Web 版无法复用 |
| Replaceable Parts | 存储、AI、向量检索可替换 | 如果早期 schema 不记录版本，迁移困难 |
